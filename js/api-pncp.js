...
// API de Integração (portal) - usada para buscar ITENS das contratações/compras
const API_PNCP_INTEGRACAO_BASE = "https://pncp.gov.br/pncp-api/v1";
...
// ====== ITENS (PNCP Integração) ======
// Cache em memória para evitar múltiplas chamadas iguais.
_itensCache: new Map(),

// Monta chave de cache: cnpj|ano|seq
_itensKey: (cnpj, ano, seq) => `${String(cnpj || '').trim()}|${String(ano || '').trim()}|${String(seq || '').trim()}`,

// Busca itens de uma "compra/contratação" no endpoint do portal:
// GET /pncp-api/v1/orgaos/{cnpj}/compras/{ano}/{sequencial}/itens
// Retorna um array (pode vir como { data: [...] } ou direto [...], dependendo do backend).
fetchCompraItens: async (cnpj, ano, seq, options = {}) => {
  const c = String(ApiPNCP.onlyDigits(cnpj || '') || '').trim();
  const a = String(ano || '').trim();
  const s = String(seq || '').trim();

  if (!c || c.length !== 14 || !a || !s) return [];

  const key = ApiPNCP._itensKey(c, a, s);
  if (ApiPNCP._itensCache.has(key)) {
    return ApiPNCP._itensCache.get(key) || [];
  }

  const { signal, timeoutMs = 20000 } = options || {};
  const url = `${API_PNCP_INTEGRACAO_BASE}/orgaos/${c}/compras/${a}/${s}/itens`;
  ...
  try {
    ...
    ApiPNCP._itensCache.set(key, arr);
    return arr;
  } catch (e) {
    // Não quebra a experiência se o endpoint falhar (ex.: indisponibilidade momentânea).
    ApiPNCP._itensCache.set(key, []);
    return [];
  }
},
