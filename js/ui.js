...
const EXCLUDE_TERMS = [
  ...
  "guarda e a seguranca",
  "guarda e a segurança"
];
...
function scoreDoctorVacancy(text) {
  ...
  // Guardrail: bloquear documentos de SST/medicina ocupacional (PGR/LTCAT/PPP/NR) que NÃO são vagas médicas.
  const safetyStrong = ['pgr','ltcat','ppp','nr1','nr-1','seguranca do trabalho','segurança do trabalho','pcmso','ppra','insalubridade','periculosidade'];
...
async function ensureItensLoadedForVacancy(item, domTargetId, signal) {
  ...
  try {
    if (!item || item._itensLoaded) return;
