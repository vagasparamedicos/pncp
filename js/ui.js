...
async function ensureItensLoadedForVacancy(item, domTargetId, signal) {
  try {
    ...
    if (_itensLoadInFlight.has(cacheKey)) {
      ...
      // já terá preenchido por referência (item pode ser outro objeto, mas ok)
      return;
    }
    ...
    card.innerHTML = `
      <div>
        ...
        <div class="mt-3 bg-white border border-slate-100 rounded-lg p-3" id="${itensDomId}">
          <div class="text-xs text-slate-500">Carregando itens (salário/plantão/valor de consulta)...</div>
        </div>
      </div>
