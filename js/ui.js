...
function scoreDoctorVacancy(text) {
  ...
  // Guardrail: documentos de PGR/LTCAT/PPP (medicina do trabalho/segurança) NÃO são vagas médicas.
  const safetyStrong = ['pgr','ltcat','ppp','nr1','nr-1','seguranca do trabalho','segurança do trabalho','insalubridade','periculosidade','ppra','pcmso'];
  const hasSafetyStrong = safetyStrong.some(x => t.includes(x));
  const doctorHiringStrong = /\bplant[aã]o\b/.test(t) || t.includes('plantonista') || t.includes('credenciamento') || t.includes('chamamento publico') || t.includes('vaga') || t.includes('vagas') || t.includes('servicos medicos') || t.includes('servico medico');
  if (hasSafetyStrong && !doctorHiringStrong) return { ok: false, score: 0 };
  ...
  const ok = Boolean(hasDoctor && hasHiring && !hasExclude && score >= 5);
  return { ok, score };
}
...
// ====== ITENS (salário / plantão / valor consulta) ======

// Extrai cnpj/ano/seq do número de controle PNCP (edital/contratação) para buscar itens no endpoint do portal.
function parseCompraKeyFromItem(item) {
  const idAny = String(
    ApiPNCP.pick(item, [
      "numeroControlePNCP",
      "numeroControlePncp",
  ...
  // Contratação/Edital: 99999999999999-1-999999/9999
  let m = idAny.match(/(\d{14})-1-(\d{1,6})\/(\d{4})/);
  if (m) return { cnpj: m[1], seq: normalizeSeq(m[2]), ano: m[3] };

  // Contrato: 99999999999999-2-999999/9999
  m = idAny.match(/(\d{14})-2-(\d{1,6})\/(\d{4})/);
  if (m) return { cnpj: m[1], seq: normalizeSeq(m[2]), ano: m[3] };

  // Ata: 99999999999999-1-999999/9999-999999 -> itens são da compra (seqCompra/ano)
  m = idAny.match(/(\d{14})-1-(\d{1,6})\/(\d{4})-(\d{1,6})/);
  if (m) return { cnpj: m[1], seq: normalizeSeq(m[2]), ano: m[3] };
...
function extractSignalsFromText(txt) {
  const t = normalizePtText(txt || "");
  const raw = String(txt || "");
  const out = { salario: "", valorConsulta: "", plantao: "", cargaHoraria: "" };

  // moeda (captura valores tipo R$ 12.345,67 / 5000 / 500,00)
  const moneyMatches = raw.match(/R\$\s*\d{1,3}(?:\.\d{3})*(?:,\d{2})?|R\$\s*\d+(?:,\d{2})?/gi) || [];
  const money = moneyMatches.map(s => s.replace(/\s+/g, ' ').trim());

  // carga horária
  const ch = raw.match(/\b(\d{1,3})\s*h(?:oras?)?\b/gi) || [];
  const ch2 = raw.match(/\b(\d{1,3})\s*horas?\s*semana(?:is)?\b/gi) || [];
  const ch3 = raw.match(/\b(\d{1,3})\s*h\s*\/\s*sem\b/gi) || [];
  const carga = [...ch, ...ch2, ...ch3].map(s => s.trim());

  // plantão
  const pl = raw.match(/\bplant[aã]o\b[^\n\r,;]{0,40}/gi) || [];
  const pl2 = raw.match(/\b(12x36|24x72|24x48|24x120|24x96|12\s*h|24\s*h)\b/gi) || [];
  const plantao = [...pl, ...pl2].map(s => s.replace(/\s+/g,' ').trim());

  // heurística salário / consulta
  if (t.includes("salario") || t.includes("remuneracao") || t.includes("vencimento") || t.includes("subsidi")) {
    out.salario = money[0] || "";
  }
  if (t.includes("consulta") || t.includes("valor da consulta") || t.includes("por consulta") || t.includes("atendimento")) {
    out.valorConsulta = money[0] || "";
  }
  if (t.includes("plantao") || plantao.length) {
    out.plantao = plantao[0] || "";
  }
  if (t.includes("carga horaria") || carga.length) {
    out.cargaHoraria = carga[0] || "";
  }

  // fallback: se existir dinheiro e nada foi classificado, guarda como "possível remuneração"
  if (!out.salario && !out.valorConsulta && money.length) out.salario = money[0];

  return out;
}

function summarizeItens(itens) {
  if (!Array.isArray(itens) || itens.length === 0) return { resumo: "", sinais: { salario:"", valorConsulta:"", plantao:"", cargaHoraria:"" } };

  const lines = [];
  let signals = { salario:"", valorConsulta:"", plantao:"", cargaHoraria:"" };

  for (const it of itens.slice(0, 6)) {
    const desc = String(ApiPNCP.pick(it, ["descricao", "descricaoItem", "descricaoItemCompra", "objeto", "nome"]) || it.descricao || it.descricaoItem || "").trim();
    const qtd = ApiPNCP.pick(it, ["quantidade", "qtde", "qtd"]) || "";
    const vu = ApiPNCP.pick(it, ["valorUnitarioEstimado", "valorUnitario", "vlrUnitario", "valorUnitarioHomologado"]) || "";
    const vt = ApiPNCP.pick(it, ["valorTotalEstimado", "valorTotal", "vlrTotal"]) || "";

    const moneyVu = vu !== "" ? moneyToBR(vu) : "";
    const moneyVt = vt !== "" ? moneyToBR(vt) : "";

    const parts = [];
    if (desc) parts.push(desc);
    if (qtd !== "") parts.push(`Qtd: ${qtd}`);
    if (moneyVu) parts.push(`Unit.: ${moneyVu}`);
    if (moneyVt) parts.push(`Total: ${moneyVt}`);

    if (parts.length) lines.push(parts.join(" • "));

    // extrair sinais a partir da descrição do item
    const sig = extractSignalsFromText(desc);
    signals.salario = signals.salario || sig.salario;
    signals.valorConsulta = signals.valorConsulta || sig.valorConsulta;
    signals.plantao = signals.plantao || sig.plantao;
    signals.cargaHoraria = signals.cargaHoraria || sig.cargaHoraria;
  }

  const resumo = lines.join("\n");
  return { resumo, sinais: signals };
}

// Carrega itens sob demanda (sem travar a tela). Atualiza apenas a área do card.
const _itensLoadInFlight = new Map(); // key -> Promise
const _itensCache = new Map(); // key -> { itens, resumo, sinais }
async function ensureItensLoadedForVacancy(item, domTargetId, signal) {
  const applyCachedToDom = (cacheKey, domId, targetItem) => {
    const host = document.getElementById(domId);
    const cached = _itensCache.get(cacheKey);
    if (!host) return;
    if (!cached) return;
    // sincroniza no item (para exportação/uso futuro)
    if (targetItem) {
      targetItem.pncpItens = cached.itens || [];
      targetItem._itensResumo = cached.resumo || '';
      targetItem._itensSinais = cached.sinais || {};
      targetItem._itensLoaded = true;
    }
    const resumo = String(cached.resumo || '').trim();
    if (!resumo) {
      host.innerHTML = `<div class=\"text-xs text-slate-500\">Itens não disponíveis para este documento.</div>`;
      return;
    }
    const s = cached.sinais || {};
    const pills = [];
    if (s.salario) pills.push(`<span class=\"inline-flex items-center px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 text-[11px] font-semibold\">Remuneração: ${escapeHtmlAttr(s.salario)}</span>`);
    if (s.valorConsulta) pills.push(`<span class=\"inline-flex items-center px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-100 text-[11px] font-semibold\">Consulta: ${escapeHtmlAttr(s.valorConsulta)}</span>`);
    if (s.plantao) pills.push(`<span class=\"inline-flex items-center px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-100 text-[11px] font-semibold\">Plantão: ${escapeHtmlAttr(s.plantao)}</span>`);
    if (s.cargaHoraria) pills.push(`<span class=\"inline-flex items-center px-2 py-0.5 rounded-full bg-slate-50 text-slate-700 border border-slate-200 text-[11px] font-semibold\">Carga: ${escapeHtmlAttr(s.cargaHoraria)}</span>`);
    const itensHtml = resumo.split("\n").map(l => `<li class=\"text-xs text-slate-700 leading-relaxed\">${escapeHtmlAttr(l)}</li>`).join("");
    host.innerHTML = `${pills.length ? `<div class=\"flex flex-wrap gap-2 mb-2\">${pills.join("")}</div>` : ""}`
      + `<div class=\"text-xs font-bold text-slate-700 mb-1\">Itens (PNCP)</div>`
      + `<ul class=\"list-disc pl-5 space-y-1\">${itensHtml}</ul>`;
  };
  try {
    if (!item) return;
    const keyObj = parseCompraKeyFromItem(item);
    if (!keyObj) {
      item._itensLoaded = true;
      return;
    }

    const cacheKey = `${keyObj.cnpj}|${keyObj.ano}|${keyObj.seq}`;
    // Se o item já foi carregado anteriormente, mas a lista foi re-renderizada (filtros/ordenação),
    // o DOM alvo muda. Então reaplicamos imediatamente o cache/HTML.
    if (item._itensLoaded) {
      if (!_itensCache.has(cacheKey) && (item._itensResumo || item.pncpItens)) {
        _itensCache.set(cacheKey, { itens: item.pncpItens || [], resumo: item._itensResumo || '', sinais: item._itensSinais || {} });
      }
      if (_itensCache.has(cacheKey)) applyCachedToDom(cacheKey, domTargetId, item);
      return;
    }

    if (_itensCache.has(cacheKey)) {
      applyCachedToDom(cacheKey, domTargetId, item);
      return;
    }

    if (_itensLoadInFlight.has(cacheKey)) {
      await _itensLoadInFlight.get(cacheKey);
      // após re-render (ordenar/filtrar), o DOM-alvo muda: aplica o cache no novo host
      applyCachedToDom(cacheKey, domTargetId, item);
      return;
    }

    const p = (async () => {
      const itens = await ApiPNCP.fetchCompraItens(keyObj.cnpj, keyObj.ano, keyObj.seq, { signal, timeoutMs: 20000 });
      item.pncpItens = itens || [];
      const { resumo, sinais } = summarizeItens(item.pncpItens);
      item._itensResumo = resumo;
      item._itensSinais = sinais;
      item._itensLoaded = true;
      _itensCache.set(cacheKey, { itens: item.pncpItens || [], resumo, sinais });

      // atualiza DOM (se ainda existir)
      const host = document.getElementById(domTargetId);
      if (host) {
        if (!resumo) {
          host.innerHTML = `<div class="text-xs text-slate-500">Itens não disponíveis para este documento.</div>`;
          return;
        ...
        const itensHtml = resumo
          .split("\n")
          .map(l => `<li class="text-xs text-slate-700 leading-relaxed">${escapeHtmlAttr(l)}</li>`)
          .join("");

        host.innerHTML = `
          ${pills.length ? `<div class="flex flex-wrap gap-2 mb-2">${pills.join("")}</div>` : ""}
          <div class="text-xs font-bold text-slate-700 mb-1">Itens (PNCP)</div>
          <ul class="list-disc pl-5 space-y-1">${itensHtml}</ul>
        `;
      }
    ...
    _itensLoadInFlight.set(cacheKey, p);
    await p;
  } catch (_) {
    // silencioso
    item._itensLoaded = true;
    const host = document.getElementById(domTargetId);
    if (host) host.innerHTML = `<div class="text-xs text-slate-500">Itens indisponíveis no momento.</div>`;
  } finally {
    try {
      const keyObj = parseCompraKeyFromItem(item);
      if (keyObj) _itensLoadInFlight.delete(`${keyObj.cnpj}|${keyObj.ano}|${keyObj.seq}`);
    } catch (_) {}
  }
}
...
function renderVacancies() {
  ...
  vacancies.forEach((item, idx) => {
    ...
    const _keyObj = parseCompraKeyFromItem(item);
    const _keyStr = _keyObj ? `${_keyObj.cnpj}-${_keyObj.ano}-${_keyObj.seq}` : String(idx);
    const itensDomId = `itens-${idx}-${ApiPNCP.onlyDigits(_keyStr) || idx}`;
    ...
    card.innerHTML = `
      <div>
        ...
        <div class="mt-3 bg-white border border-slate-100 rounded-lg p-3" id="${itensDomId}">
          <div class="text-xs text-slate-500">Carregando itens...</div>
        </div>
      </div>
    ...
    // carrega itens sob demanda (não bloqueia a lista)
    ensureItensLoadedForVacancy(item, itensDomId, currentAbortCtrl ? currentAbortCtrl.signal : undefined);
  });
}
...
function exportVacanciesExcel() {
  ...
  const rows = vacancies.map((item, idx) => {
    ...
    return {
      ...
      "Itens (resumo)": String(item._itensResumo || "").trim(),
      "Remuneração (extraída)": String((item._itensSinais && (item._itensSinais.salario || item._itensSinais.valorConsulta)) || "").trim(),
      "Plantão (extraído)": String((item._itensSinais && item._itensSinais.plantao) || "").trim(),
      "Carga horária (extraída)": String((item._itensSinais && item._itensSinais.cargaHoraria) || "").trim(),
      "Link": link
    };
  });
